...
const GLYPHS = {
	...
	loading: '~',
	/*
	 * A plugin: a box with something added into it. The same family as the
	 * browser tab's `⊕` — a frame with a mark in it — because both are things
	 * that arrive from outside and take a place in the dock.
	 */
	plugins: '⊞',
	pin: '■',
	'primitive-square': '■',
 ...
 * than a blank space — which is why `Icon`'s props take this union and not
 * `string`, and why `TreeNode`, `IconButton` and the artifact table take it too.
 */
export type IconName = keyof typeof GLYPHS;

/**
 * The same names as a list, for the one caller that needs to *enumerate* them
 * rather than check one: `publicNames.ts`, which promises them to plugins.
 */
