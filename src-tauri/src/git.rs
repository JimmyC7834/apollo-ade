...
///
/// Both sides are optional: an added file has no HEAD blob and a deleted file
/// has no working-tree file, so a missing side is empty rather than an error.
/// The working tree is read through `resolve`, which brings the workspace's
/// symlink, size and confinement rules with it.
///
